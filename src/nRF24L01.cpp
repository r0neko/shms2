...
void RFProtocol::write_data(unsigned char* data, size_t d_len) {
	((nRF24L01*)this)->write(&data, d_len, 0);
}

void nRF24L01::csn(bool mode)
{
    if(!mode)
      _SPI.chipSelect(csn_pin);
}

void nRF24L01::ce(bool level)
{
...
/****************************************************************************/
  ...
  inline void nRF24L01::beginTransaction() {
...
/****************************************************************************/
  ...
  inline void nRF24L01::endTransaction() {
    csn(HIGH);
    _SPI.endTransaction();
  }
...
uint8_t nRF24L01::write_payload(const void* buf, uint8_t data_len, const uint8_t writeType)
{
  uint8_t status;
...
uint8_t nRF24L01::read_payload(void* buf, uint8_t data_len)
{
  uint8_t status;
...
nRF24L01::nRF24L01(): p_variant(false), payload_size(32), dynamic_payloads_enabled(false),addr_width(5)//,pipe0_reading_address(0) 
{
  ce_pin = RPI_V2_GPIO_P1_22;
  csn_pin = RPI_V2_GPIO_P1_24;
  spi_speed = BCM2835_SPI_SPEED_8MHZ;
...
static const char RF24_model_e_str_0[] PROGMEM = "nnRF24L01L01";
static const char RF24_model_e_str_1[] PROGMEM = "nnRF24L01L01+";
...
bool nRF24L01::begin(void)
{
  ...
  // check for connected module and if this is a p nnRF24L01l01 variant
  //
  ...
  /*if( setup == 0b00001110 )     // register default for nnRF24L01L01P
  {
    p_variant = true ;
  }*/
...
void nRF24L01::powerUp(void)
{
   ...
   if (!(cfg & _BV(PWR_UP))){
      ...
      // For nnRF24L01L01+ to go from power down mode to TX or RX mode it must first pass through stand-by mode.
    // There must be a delay of Tpd2stby (see Table 16.) after the nnRF24L01L01+ leaves power down mode before
...
{
  // Note that AVR 8-bit uC's store this LSB first, and the NnRF24L01L01(+)
